...
#include <string.h>
#include <png.h>
#include "com_khan_captchareader_rarbgcaptcha_RarBGCaptchaReader.h"
...
JNIEXPORT jstring JNICALL Java_com_khan_captchareader_rarbgcaptcha_RarBGCaptchaReader_readCaptha(JNIEnv * env, jobject jobj, jstring jstr_path){
	const char *image_path = env->GetStringUTFChars(jstr_path, 0);
	string image_val=readRARBGCaptchaFile(image_path);
	return env->NewStringUTF(image_val.c_str());
}
